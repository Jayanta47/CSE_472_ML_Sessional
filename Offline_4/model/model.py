...
from utils.util_functions import get_batches, evaluate
        ...
        for epoch in range(epochs):
            ...
            for i, (x_batch, y_batch) in enumerate(get_batches(data, labels)):
                ...
                for num, layer in enumerate(self.model):
                    batch_preds = layer.forward(batch_preds, save_cache=True)
            ...
            iter += batch_size
            print("Epoch: {epoch}, Iteration: {iter}, Loss: {loss}".format(epoch=epoch + 1, iter=iter, loss=loss))
